...
macro_rules! unexpected_type {
    ($name:expr, $value:expr) => {
        Err(RecordConversionError::UnexpectedType(format!(
            "expected {} but got {:?}",
            $name, $value
        )))
    };
...
impl From<arrow::error::ArrowError> for RecordConversionError {
    fn from(err: arrow::error::ArrowError) -> Self {
        RecordConversionError::ArrowError(err)
    }
}
...
pub fn append_record(
    ...
    match schema {
        ...
        Schema::Decimal(_) => {
            convert!(builder, Decimal, Decimal128Builder, record, from_decimal128)
        }

        Schema::Date => convert!(builder, Date, Date32Builder, record, deref),

        Schema::TimeMillis => {
            convert!(builder, TimeMillis, Time32MillisecondBuilder, record, deref)
        }

        Schema::TimeMicros => {
            convert!(builder, TimeMicros, Time64MicrosecondBuilder, record, deref)
        }
        ...
        Schema::String => {
            convert!(builder, String, StringBuilder, record, asis)
        }
        ...
        Schema::Bytes => convert!(builder, Bytes, BinaryBuilder, record, asis),
        ...
        Schema::Union(inner) => {
            ...
            match record {
                Value::Union(_, value) => append_record(builder, type_schema, value),
                _ => unexpected_type!("Union", record),
            }
        }

        _ => Err(RecordConversionError::UnsupportedType(format!(
            "unsupported schema {:?}",
            schema
        ))),
    }
