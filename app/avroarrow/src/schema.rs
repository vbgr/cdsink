...
pub enum SchemaConversionError {
    ...
    ResolutionError(apache_avro::schema::Name),
    #[error("Avro error: {0}")]
    AvroError(String),
    #[error("Arrow error: {0}")]
...
/// Macro for creating Avro error messages
/// Usage:
/// - `avro_err!("message")` -> "message".into()
/// - `avro_err!("format {}", arg)` -> format!("format {}", arg).into()
macro_rules! avro_err {
    ($msg:expr) => {
        Err(SchemaConversionError::AvroError($msg.into()))
    };
    ($fmt:expr, $($arg:tt)*) => {
        Err(SchemaConversionError::AvroError(format!($fmt, $($arg)*).into()))
    };
}

pub fn convert_schema(
    src: &AvroSchema,
    registry: &HashMap<apache_avro::schema::Name, AvroSchema>,
) -> Result<ArrowSchema, SchemaConversionError> {
    match src {
        AvroSchema::Record(record_schema) => {
            let arrow_fields: Result<Vec<Field>, SchemaConversionError> = record_schema
                .fields
                .iter()
                .map(|field| {
                    Ok(Field::new(
                        field.name.clone(),
                        convert_to_datatype(&field.schema, registry)?,
        ...
        _ => avro_err!("expected Record but got {}", src),
    }
}

fn convert_to_datatype(
    src: &AvroSchema,
    registry: &HashMap<apache_avro::schema::Name, AvroSchema>,
) -> Result<DataType, SchemaConversionError> {
    match src {
        ...
        AvroSchema::LocalTimestampMillis => Ok(arrow_local_timestamp_type(TimeUnit::Millisecond)),
        AvroSchema::LocalTimestampMicros => Ok(arrow_local_timestamp_type(TimeUnit::Microsecond)),
        AvroSchema::LocalTimestampNanos => Ok(arrow_local_timestamp_type(TimeUnit::Nanosecond)),
        ...
        AvroSchema::Decimal(schema) => Ok(DataType::Decimal128(
            schema.precision as u8,
            schema.scale as i8,
        )),
        AvroSchema::Fixed(schema) => Ok(DataType::FixedSizeBinary(schema.size as i32)),
        AvroSchema::BigDecimal => Ok(DataType::Binary),
        AvroSchema::Array(schema) => Ok(DataType::List(Arc::new(create_array_field(
            schema, registry,
        )?))),
            ...
            let entries = Field::new(
                ...
                DataType::Struct(
                    vec![
                        ...
                        create_map_values_field(schema, registry)?,
                    ]
                    .into(),
                ),
                false,
        ...
        AvroSchema::Union(schema) => {
            if is_optional(schema) {
                convert_to_datatype(&schema.variants()[1], registry)
            } else {
                avro_err!("only unions of king [null, TYPE] are supported")
            }
        }
        AvroSchema::Record(schema) => {
            let arrow_fields: Result<Fields, SchemaConversionError> = schema
                .fields
                .iter()
                .map(|field| {
        ...
        AvroSchema::Ref { name } => {
            if let Some(schema) = registry.get(name) {
                convert_to_datatype(schema, registry)
            } else {
                Err(SchemaConversionError::ResolutionError(name.clone()))
            }
        }
    }
}
...
pub fn create_builder(
    ...
    cap: usize,
    registry: &HashMap<apache_avro::schema::Name, AvroSchema>,
) -> Result<Box<dyn ArrayBuilder>, SchemaConversionError> {
    match avro {
        ...
        AvroSchema::Fixed(schema) => Ok(Box::new(FixedSizeBinaryBuilder::with_capacity(
            cap,
            schema.size as i32,
        ))),
        ...
        AvroSchema::Array(schema) => array_builder(schema, cap, registry),
        AvroSchema::Map(schema) => map_builder(schema, cap, registry),
        AvroSchema::Record(schema) => struct_builder(schema, cap, registry),
        AvroSchema::Union(schema) => {
            if is_optional(schema) {
                create_builder(&schema.variants()[1], cap, registry)
            } else {
                avro_err!("only unions of king [null, TYPE] are supported")
            }
        }
        _ => avro_err!("cannot create builder for {:?}", avro),
    }
}
...
#[inline]
fn create_array_field(
    schema: &ArraySchema,
    registry: &HashMap<apache_avro::schema::Name, AvroSchema>,
) -> Result<Field, SchemaConversionError> {
    Ok(Field::new(
        "item",
        convert_to_datatype(&schema.items, registry)?,
        is_nullable(&schema.items),
    ))
}

#[inline]
fn create_map_values_field(
    schema: &MapSchema,
    registry: &HashMap<apache_avro::schema::Name, AvroSchema>,
) -> Result<Field, SchemaConversionError> {
    Ok(Field::new(
        "values",
        convert_to_datatype(&schema.types, registry)?,
...
fn array_builder(
    ...
    let builder = create_builder(&schema.items, cap, registry)?;
    let result =
        ListBuilder::with_capacity(builder, cap).with_field(create_array_field(schema, registry)?);
    Ok(Box::new(result))
}
...
fn map_builder(
    ...
    let values_builder = create_builder(schema.types.as_ref(), cap, registry)?;
    let result = MapBuilder::new(None, keys_builder, values_builder)
        .with_values_field(create_map_values_field(schema, registry)?);
...
fn struct_builder(
    ...
    for f in &schema.fields {
        let t = convert_to_datatype(&f.schema, registry)?;
        fields.push(Field::new(f.name.clone(), t, is_nullable(&f.schema)));
        builders.push(create_builder(&f.schema, cap, registry)?);
    }
