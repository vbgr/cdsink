...
fn to_arrow<T: ArrayBuilder>(schema: &Schema, values: &Vec<Value>) -> ArrayRef {
    let registry = HashMap::<apache_avro::schema::Name, apache_avro::Schema>::default();
    let mut builder = avroarrow::create_builder(&schema, 32, &registry).unwrap();
    for v in values {
        avroarrow::append_record(&mut builder, &schema, &v).unwrap();
    }

    builder.as_any_mut().downcast_mut::<T>().unwrap().finish()
}

fn record_batch_to_json(record_batch: &arrow::record_batch::RecordBatch) -> serde_json::Value {
    let buffer = Vec::new();
...
fn test_append_decimal() {
    ...
    let expected: ArrayRef =
        Arc::new(Decimal128Array::from(raw).with_data_type(DataType::Decimal128(10, 4)));
...
fn test_append_timestamp_ms() {
    let raw: Vec<i64> = vec![1709821534123, 1709821533123, 1709821532123];
...
fn test_append_timestamp_mc() {
    let raw: Vec<i64> = vec![1709821534123123, 1709821533123456, 1709821532123789];
...
fn test_append_local_timestamp_ms() {
    ...
    let values: Vec<_> = raw
        .iter()
        .map(|v| Value::LocalTimestampMillis(*v))
        .collect();

    let expected: ArrayRef = Arc::new(
        TimestampMillisecondArray::from(raw)
            .with_data_type(DataType::Timestamp(TimeUnit::Millisecond, tz_offset())),
    );
    let actual = to_arrow::<TimestampMillisecondBuilder>(&schema, &values);
...
fn test_append_local_timestamp_mc() {
    ...
    let values: Vec<_> = raw
        .iter()
        .map(|v| Value::LocalTimestampMicros(*v))
        .collect();

    let expected: ArrayRef = Arc::new(
        TimestampMicrosecondArray::from(raw)
            .with_data_type(DataType::Timestamp(TimeUnit::Microsecond, tz_offset())),
    );
    let actual = to_arrow::<TimestampMicrosecondBuilder>(&schema, &values);
...
fn test_append_local_timestamp_ns() {
    ...
    let expected: ArrayRef = Arc::new(
        TimestampNanosecondArray::from(raw)
            .with_data_type(DataType::Timestamp(TimeUnit::Nanosecond, tz_offset())),
    );
    let actual = to_arrow::<TimestampNanosecondBuilder>(&schema, &values);
...
#[test]
fn test_append_enum() {
    let raw = vec!["X1".to_string(), "X2".to_string(), "X3".to_string()];
...
fn test_append_uuid() {
    ...
    let values: Vec<_> = raw
        .iter()
        .map(|v| Value::Uuid(uuid::Uuid::parse_str(*v).unwrap()))
        .collect();

    let bin = raw
        .iter()
        .map(|v| Some(uuid::Uuid::parse_str(v).unwrap().as_bytes().clone()))
        .collect::<Vec<Option<_>>>();

    let expected: ArrayRef = Arc::new(
        FixedSizeBinaryArray::try_from_sparse_iter_with_size(bin.into_iter(), 16).unwrap(),
    );
...
fn test_append_bytes() {
    let raw: Vec<&[u8]> = vec![b"one", b"tow", b"", b"three"];

    let schema = Schema::Bytes;
    let values: Vec<Value> = raw.iter().map(|v| Value::Bytes(v.to_vec())).collect();
...
fn test_append_fixed() {
    let raw: Vec<&[u8; 2]> = vec![b"12", b"23", b"56"];
    ...
    let values: Vec<Value> = raw.iter().map(|v| Value::Fixed(2, v.to_vec())).collect();

    let bin = raw.iter().map(|v| Some(v)).collect::<Vec<Option<_>>>();

    let expected: ArrayRef =
        Arc::new(FixedSizeBinaryArray::try_from_sparse_iter_with_size(bin.into_iter(), 2).unwrap());
    let actual = to_arrow::<FixedSizeBinaryBuilder>(&schema, &values);
...
fn test_append_string_map() {
    ...
    let actual = to_arrow::<MapBuilder<StringBuilder, Box<dyn ArrayBuilder>>>(&schema, &values);
...
fn test_append_struct() {
    ...
    let arrow_schema = avroarrow::convert_schema(&schema, &registry).unwrap();
