...
fn test_convert_schema_flat() {
    let avro_schema_str = std::fs::read_to_string("./tests/data/schema/example.avsc").unwrap();
    ...
    let actual = avroarrow::convert_schema(&avro_schema, &registry).unwrap();
...
fn test_convert_schema_nested() {
    let avro_schemas_str = vec![
        std::fs::read_to_string("./tests/data/schema/contact.avsc").unwrap(),
        std::fs::read_to_string("./tests/data/schema/address.avsc").unwrap(),
        std::fs::read_to_string("./tests/data/schema/person.avsc").unwrap(),
    ];

    let avro_schemas_ref: Vec<&str> = avro_schemas_str.iter().map(|s| s.as_str()).collect();

    let avro_schemas = AvroSchema::parse_list(&avro_schemas_ref).unwrap();
    let person_schema = &avro_schemas[2];

    let registry = hashmap! {
        avro_schemas[0].name().unwrap().clone() => avro_schemas[0].clone(),
        avro_schemas[1].name().unwrap().clone() => avro_schemas[1].clone(),
        avro_schemas[2].name().unwrap().clone() => avro_schemas[2].clone(),
    };

    let arrow_contact_schema = DataType::Struct(
        vec![
    ...
    let arrow_address_schema = DataType::Struct(
        vec![
    ...
    let arrow_job_schema = DataType::Struct(
        vec![
    ...
    let actual = avroarrow::convert_schema(&person_schema, &registry).unwrap();
...
fn test_create_builder_flat() {
    ...
    let registry = HashMap::<apache_avro::schema::Name, apache_avro::Schema>::default();
    avroarrow::create_builder(&avro_schema, 100, &registry).unwrap();
}
